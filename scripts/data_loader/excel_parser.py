...
Reads the bilingual training data from Excel or Google Sheets and produces:
  - A list of (text, intent) training pairs with [EN]/[JA] prefixes
  - A DataFrame with answer templates per intent
...
def _flatten_to_train_rows(df):
    ...
    Expects columns: ['intent', 'type', 'q_en', 'q_ja', 'a_en', 'a_ja']
    """
    # Forward-fill intent and type for grouped rows
...
def parse_excel(excel_path):
    ...
    Expected Excel layout (multi-level header):
        intent | type | Question (English, Japanese) | Answer (English, Japanese)
...
def parse_google_sheet(sheet_url=None, sheet_name=None):
    ...
    The sheet must have the same layout as the Excel template:
        Row 1: intent | type | Question |          | Answer  |
    ...
    df = pd.DataFrame(data_rows, columns=['intent', 'type', 'q_en', 'q_ja', 'a_en', 'a_ja'])
